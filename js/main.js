...
function hideAlerts() {
  for (var i = 0; i < alerts.length; i++) alerts[i].style.display = "none";
}
...
function isDrawn(bookmark) {
  for (var i = 0; i < drawn.length; i++) if (bookmark == drawn[i]) return true;
  return false;
}

function displayData() {
  for (var i = 0; i < bookmarks.length; i++) {
    if (!isDrawn(bookmarks[i])) createWell(bookmarks[i]);
  }
}
...
function createWell(bookmark) {
  ...
  div.innerHTML +=
    '<div class="webwell row" id="' + bookmark.name + '"></div> ';
  var link =
...
function checkUrl(url) {
  ...
  for (var i = 0; i < bookmarks.length; i++) {
    if (bookmarks[i].url === url) return false;
  }
  return true;
}
